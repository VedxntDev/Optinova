...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Eye Tracking Component (@componentry/eye-tracking) */
        .eye-tracker-panel {
        ...
        .eye-tracker-body {
            ...
            gap: 20px;
            padding: 28px 24px;
            background: radial-gradient(circle at 50% 50%, rgba(217, 119, 6, 0.03) 0%, transparent 70%);
        }
        ...
        .reticle-ring-inner {
            ...
            right: 15px;
            bottom: 15px;
            border: 1px dotted rgba(217, 119, 6, 0.35);
            border-radius: 50% !important;
        }
        ...
        .reticle-0 { top: 2px; left: 50%; transform: translateX(-50%); }
        .reticle-90 { right: 2px; top: 50%; transform: translateY(-50%); }
        .reticle-180 { bottom: 2px; left: 50%; transform: translateX(-50%); }
        .reticle-270 { left: 2px; top: 50%; transform: translateY(-50%); }
        ...
        .eye-socket {
            ...
            border-radius: 50% !important;
            overflow: hidden;
            box-shadow: inset 0 0 16px rgba(0, 0, 0, 0.45), 0 0 0 1px var(--border-color);
            display: flex;
            align-items: center;
            justify-content: center;
        }

        [data-theme="light"] .eye-socket {
            background: #f1f5f9;
            box-shadow: inset 0 0 12px rgba(0, 0, 0, 0.2), 0 0 0 1px var(--border-color);
        }
        ...
        .eye-sclera {
            ...
            height: 100%;
            border-radius: 50% !important;
            background: radial-gradient(circle, #f8fafc 40%, #e2e8f0 85%, #cbd5e1 100%);
            display: flex;
            align-items: center;
            justify-content: center;
        }

        [data-theme="dark"] .eye-sclera {
            background: radial-gradient(circle, #e2e8f0 40%, #94a3b8 85%, #64748b 100%);
        }
        ...
        .eye-iris {
            ...
            height: 60px;
            border-radius: 50% !important;
            background: radial-gradient(circle at 35% 35%, #f59e0b 0%, #b45309 45%, #78350f 75%, #451a03 100%);
            box-shadow: 0 0 12px rgba(217, 119, 6, 0.4), inset 0 0 6px rgba(0,0,0,0.8);
            display: flex;
            align-items: center;
            justify-content: center;
            will-change: transform;
        ...
        .iris-texture {
            ...
            height: 100%;
            border-radius: 50% !important;
            background-image: repeating-conic-gradient(from 0deg, rgba(255,255,255,0.15) 0deg 6deg, transparent 6deg 12deg);
            opacity: 0.7;
            pointer-events: none;
        }
        ...
        .eye-socket.blinking .eyelid-top {
            transform: translateY(0);
        }

        .eye-socket.blinking .eyelid-bottom {
            transform: translateY(0);
        }
        ...
        .telemetry-stat-row:last-of-type {
            border-bottom: none;
        }

        .telemetry-stat-row .t-label {
            color: var(--text-muted);
            font-weight: 600;
        }

        .telemetry-stat-row .t-val {
            color: var(--accent-gold);
            font-weight: 700;
        }

        .telemetry-stat-row .t-val.status-ok {
            color: var(--accent-emerald);
        }

        .telemetry-radar-indicator {
            width: 100%;
            height: 48px;
        ...
        .telemetry-radar-indicator::before {
            content: '';
            position: absolute;
            top: 0;
            bottom: 0;
        ...
        .telemetry-radar-indicator::after {
            content: '';
            position: absolute;
            left: 0;
            right: 0;
        ...
        .eye-tracker-footer .footer-badge span {
            color: var(--text-primary);
            font-weight: 600;
        }
...
<body>
    ...
    <section class="hero no-print">
        <div class="container">
            ...
            <!-- Ocular Gaze & Fixation Tracking Component (@componentry/eye-tracking) -->
            <div class="eye-tracker-panel" id="eyeTrackerWidget">
                <div class="eye-tracker-header">
                    <div class="eye-tracker-title">
                        <span class="pulse-dot"></span>
                        <span class="title-tag">MODULE // OCULAR GAZE &amp; FIXATION TRACKING</span>
                        <span class="sub-tag">[@componentry/eye-tracking]</span>
                    </div>
                    <div class="eye-tracker-actions">
                        <span class="eye-tracker-status" id="eyeTrackingStatus">STATUS: 240Hz FIXATION LOCKED</span>
                        <button type="button" class="btn-eye-mode" id="btnBlinkTest" onclick="triggerEyeBlink()">Simulate Saccade</button>
                        <button type="button" class="btn-eye-mode" id="btnCenterGaze" onclick="recenterGaze()">Recenter</button>
                    </div>
                </div>

                <div class="eye-tracker-body">
                    <!-- OD: Oculus Dexter (Right Eye) -->
                    <div class="eye-module" id="eyeModuleOD">
                        <div class="eye-socket-wrapper" id="socketWrapOD">
                            <div class="eye-reticle-overlay">
                                <div class="reticle-deg reticle-0">0°</div>
                                <div class="reticle-deg reticle-90">90°</div>
                                <div class="reticle-deg reticle-180">180°</div>
                                <div class="reticle-deg reticle-270">270°</div>
                                <div class="reticle-axis-x"></div>
                                <div class="reticle-axis-y"></div>
                                <div class="reticle-ring-inner"></div>
                            </div>
                            <div class="eye-socket" id="eyeSocketOD">
                                <div class="eye-sclera" id="scleraOD">
                                    <div class="eye-iris" id="irisOD">
                                        <div class="iris-texture"></div>
                                        <div class="eye-pupil" id="pupilOD">
                                            <div class="corneal-glint glint-1"></div>
                                            <div class="corneal-glint glint-2"></div>
                                        </div>
                                    </div>
                                </div>
                                <div class="eyelid eyelid-top" id="eyelidTopOD"></div>
                                <div class="eyelid eyelid-bottom" id="eyelidBottomOD"></div>
                            </div>
                        </div>
                        <div class="eye-label-tag">
                            <span class="laterality">OD</span>
                            <span class="label-desc">OCULUS DEXTER (RIGHT EYE)</span>
                        </div>
                    </div>

                    <!-- Central Telemetry Cross-Section -->
                    <div class="eye-telemetry-center">
                        <div class="telemetry-stat-row">
                            <span class="t-label">GAZE VECTOR (θ, φ)</span>
                            <span class="t-val" id="gazeVectorVal">+0.0° , +0.0°</span>
                        </div>
                        <div class="telemetry-stat-row">
                            <span class="t-label">CORNEAL REFLECTION (P1)</span>
                            <span class="t-val status-ok" id="cornealP1Val">PURKINJE LOCKED (99.4%)</span>
                        </div>
                        <div class="telemetry-stat-row">
                            <span class="t-label">FIXATION DISPERSION</span>
                            <span class="t-val" id="fixationDispVal">BCEA: 0.14 deg² (STABLE)</span>
                        </div>
                        <div class="telemetry-stat-row">
                            <span class="t-label">PUPIL APERTURE</span>
                            <span class="t-val" id="pupilApertureVal">OD: 4.2mm | OS: 4.2mm</span>
                        </div>
                        <div class="telemetry-radar-indicator">
                            <div class="radar-dot" id="radarGazeDot"></div>
                        </div>
                    </div>

                    <!-- OS: Oculus Sinister (Left Eye) -->
                    <div class="eye-module" id="eyeModuleOS">
                        <div class="eye-socket-wrapper" id="socketWrapOS">
                            <div class="eye-reticle-overlay">
                                <div class="reticle-deg reticle-0">0°</div>
                                <div class="reticle-deg reticle-90">90°</div>
                                <div class="reticle-deg reticle-180">180°</div>
                            ...
                            <div class="eye-socket" id="eyeSocketOS">
                                <div class="eye-sclera" id="scleraOS">
                                    <div class="eye-iris" id="irisOS">
                                        <div class="iris-texture"></div>
                                        <div class="eye-pupil" id="pupilOS">
                                            <div class="corneal-glint glint-1"></div>
                                            <div class="corneal-glint glint-2"></div>
                                        </div>
                                    </div>
                                </div>
                                <div class="eyelid eyelid-top" id="eyelidTopOS"></div>
                                <div class="eyelid eyelid-bottom" id="eyelidBottomOS"></div>
                            </div>
                        </div>
                        <div class="eye-label-tag">
                            <span class="laterality">OS</span>
                            <span class="label-desc">OCULUS SINISTER (LEFT EYE)</span>
                        </div>
                    </div>
                </div>

                <div class="eye-tracker-footer">
                    <div class="footer-badge"><span>TRACKING ENGINE:</span> Realtime 240Hz sub-pixel gaze kinematics</div>
                    <div class="footer-badge"><span>CORNEAL PURKINJE:</span> Double-glint specular vector active</div>
                    <div class="footer-badge"><span>INTEGRATION:</span> @componentry/eye-tracking</div>
                </div>
            </div>
    ...
    <script>
        ...
        // Ocular Gaze & Fixation Tracking Engine (@componentry/eye-tracking)
        (function initEyeTrackingEngine() {
            const irisOD = document.getElementById('irisOD');
            const irisOS = document.getElementById('irisOS');
            const pupilOD = document.getElementById('pupilOD');
            const pupilOS = document.getElementById('pupilOS');
            const eyeSocketOD = document.getElementById('eyeSocketOD');
            const eyeSocketOS = document.getElementById('eyeSocketOS');
            const radarDot = document.getElementById('radarGazeDot');
            const gazeVectorVal = document.getElementById('gazeVectorVal');
            const pupilApertureVal = document.getElementById('pupilApertureVal');
            const fixationDispVal = document.getElementById('fixationDispVal');

            if (!irisOD || !irisOS) return;

            let targetX_OD = 0, targetY_OD = 0;
            let targetX_OS = 0, targetY_OS = 0;
            let currentX_OD = 0, currentY_OD = 0;
            let currentX_OS = 0, currentY_OS = 0;

            const MAX_RADIUS = 24; // max pupil/iris displacement in px
            let isUserInteracting = false;
            let lastMouseX = window.innerWidth / 2;
            let lastMouseY = window.innerHeight / 2;

            function computeEyeOffset(eyeElem, mouseX, mouseY) {
                ...
                // Non-linear damping for natural ocular saccade motion
                const clampedDist = Math.min(MAX_RADIUS, dist / 18);
                const offsetX = Math.cos(angle) * clampedDist;
                const offsetY = Math.sin(angle) * clampedDist;

                return { offsetX, offsetY, angle, dist, degX: (offsetX / MAX_RADIUS) * 28, degY: (offsetY / MAX_RADIUS) * 24 };
            }

            function onPointerMove(e) {
                isUserInteracting = true;
                lastMouseX = e.clientX || (e.touches && e.touches[0] ? e.touches[0].clientX : window.innerWidth / 2);
                lastMouseY = e.clientY || (e.touches && e.touches[0] ? e.touches[0].clientY : window.innerHeight / 2);
            }

            window.addEventListener('mousemove', onPointerMove, { passive: true });
            window.addEventListener('touchmove', onPointerMove, { passive: true });

            // Periodic spontaneous blink & saccade mechanics
            function triggerBlink() {
                if (eyeSocketOD && eyeSocketOS) {
                    eyeSocketOD.classList.add('blinking');
                    eyeSocketOS.classList.add('blinking');
                    setTimeout(() => {
                        eyeSocketOD.classList.remove('blinking');
                        eyeSocketOS.classList.remove('blinking');
                    }, 140);
                }
            }
            window.triggerEyeBlink = triggerBlink;

            window.recenterGaze = function() {
                targetX_OD = 0; targetY_OD = 0;
                targetX_OS = 0; targetY_OS = 0;
                lastMouseX = window.innerWidth / 2;
                lastMouseY = window.innerHeight / 2;
                triggerBlink();
            };

            // Spontaneous natural blinking interval
            ...
            // Micro-saccadic tremor & smooth tracking loop
            let frameCount = 0;
            function updateEyes() {
                frameCount++;
                
                // If idle, simulate gentle micro-fixational drift
                let mx = lastMouseX;
                let my = lastMouseY;
                if (!isUserInteracting) {
                    const time = Date.now() * 0.0015;
                    mx = window.innerWidth / 2 + Math.cos(time * 0.8) * 140;
                    my = window.innerHeight / 3 + Math.sin(time * 1.2) * 80;
                }

                const resOD = computeEyeOffset(eyeSocketOD, mx, my);
                const resOS = computeEyeOffset(eyeSocketOS, mx, my);

                targetX_OD = resOD.offsetX;
                targetY_OD = resOD.offsetY;
                targetX_OS = resOS.offsetX;
                targetY_OS = resOS.offsetY;

                // Smooth linear interpolation (lerp)
                currentX_OD += (targetX_OD - currentX_OD) * 0.14;
                currentY_OD += (targetY_OD - currentY_OD) * 0.14;
                currentX_OS += (targetX_OS - currentX_OS) * 0.14;
                currentY_OS += (targetY_OS - currentY_OS) * 0.14;

                irisOD.style.transform = `translate3d(${currentX_OD.toFixed(2)}px, ${currentY_OD.toFixed(2)}px, 0)`;
                irisOS.style.transform = `translate3d(${currentX_OS.toFixed(2)}px, ${currentY_OS.toFixed(2)}px, 0)`;

                // Update Telemetry HUD every 4 frames
                if (frameCount % 4 === 0) {
                    const avgDegX = ((resOD.degX + resOS.degX) / 2).toFixed(1);
                    const avgDegY = ((-resOD.degY - resOS.degY) / 2).toFixed(1);
                    const signX = avgDegX >= 0 ? '+' : '';
                    const signY = avgDegY >= 0 ? '+' : '';
                    
                    if (gazeVectorVal) {
                        gazeVectorVal.innerText = `${signX}${avgDegX}° , ${signY}${avgDegY}°`;
                    }

                    // Radar Indicator
                    if (radarDot) {
                        const radarPercentX = 50 + (currentX_OD / MAX_RADIUS) * 42;
                        const radarPercentY = 50 + (currentY_OD / MAX_RADIUS) * 42;
                        radarDot.style.left = `${radarPercentX.toFixed(1)}%`;
                        radarDot.style.top = `${radarPercentY.toFixed(1)}%`;
                    }

                    // Dynamic light reflex pupil scaling
                    const normGaze = Math.hypot(currentX_OD, currentY_OD) / MAX_RADIUS;
                    const dynamicPupil = (4.2 - normGaze * 0.6).toFixed(1);
                    if (pupilApertureVal) {
                        pupilApertureVal.innerText = `OD: ${dynamicPupil}mm | OS: ${dynamicPupil}mm`;
                    }
                }
